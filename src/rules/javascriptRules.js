...
function runJavaScriptRules(lines) {
    var issues = [];
    for (var i = 0; i < lines.length; i++) {
        ...
        for (var _i = 0, RULES_1 = RULES; _i < RULES_1.length; _i++) {
            ...
            while ((match = rule.pattern.exec(line)) !== null) {
                issues.push({
                    id: "".concat(rule.id, ":").concat(i, ":").concat(match.index),
    ...
    issues.push.apply(issues, detectLongFunctions(lines));
    return issues;
}
...
function detectLongFunctions(lines) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (fnStart !== -1 && depth <= startDepth) {
            ...
            if (len > 50) {
                issues.push({
                    id: "js:long-fn:".concat(fnStart), message: "Function is ".concat(len, " lines \u2014 split it up."),
